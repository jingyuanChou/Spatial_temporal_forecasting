...
def load_data(args):
    ...
    # train/val/test
    if args.data == 'HOSP':
        df = pd.read_csv('C:\\Users\\6\\PycharmProjects\\FORECASTING_GNN\\benchmark data\\HOSP\\hosp_weekly_filt_case_data.csv')
        hospitalization = df.reset_index()
        hospitalization['state'] = hospitalization['state'].astype(int)
        neighbouring_states = pd.read_csv('C:\\Users\\6\\PycharmProjects\\FORECASTING_GNN\\benchmark data\\HOSP\\neighbouring_states.csv')
        fips_states = pd.read_csv('C:\\Users\\6\\PycharmProjects\\FORECASTING_GNN\\benchmark data\\HOSP\\state_hhs_map.csv', header=None).iloc[:(-3), :]
        ...
        df = hospitalization.T
        traffic = torch.tensor(df.values)
        '''
        neighbouring_states['connected'] = 1

        # Get all unique nodes
        nodes = set(neighbouring_states['StateCode']).union(set(neighbouring_states['NeighborStateCode']))

        # Create all possible combinations of node pairs
        all_combinations = pd.DataFrame(product(nodes, nodes), columns=['StateCode', 'NeighborStateCode'])

        # Merge with the existing DataFrame
        expanded_df = all_combinations.merge(neighbouring_states, on=['StateCode', 'NeighborStateCode'], how='left')

        # Replace NaNs with 0 in the 'connected' column
        expanded_df['connected'].fillna(0, inplace=True)

        # Convert the 'connected' column to int (optional)
        expanded_df['connected'] = expanded_df['connected'].astype(int)


        expanded_df.to_csv('adj_HOSP.txt', sep='\t', index=False)
        '''
    num_step = df.shape[0]
