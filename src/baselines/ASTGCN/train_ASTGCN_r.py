...
import argparse
import configparser
from baselines.ASTGCN.model.ASTGCN_r import make_model
from baselines.ASTGCN.lib.utils import load_graphdata_channel1, get_adjacency_matrix, compute_val_loss_mstgcn, \
    predict_and_save_results_mstgcn
from tensorboardX import SummaryWriter
from baselines.ASTGCN.lib.metrics import masked_mape_np, masked_mae, masked_mse, masked_rmse
...
adj_mx, distance_mx = get_adjacency_matrix(adj_filename, num_of_vertices, dataset_name, id_filename)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            '''
        print('epoch: %s, training loss: %.2f' % (epoch, np.mean(train_loss_ls)))
...
def predict_main(global_step, data_loader, data_target_tensor, metric_method, _mean, _std, type):
    ...
    predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method, _mean, _std,
                                    params_path, type)
