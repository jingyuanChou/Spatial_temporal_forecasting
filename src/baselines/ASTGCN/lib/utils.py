...
from baselines.ASTGCN.lib.metrics import masked_mape_np,  masked_mae,masked_mse,masked_rmse,masked_mae_test,masked_rmse_test
...
def get_adjacency_matrix(distance_df_filename, num_of_vertices, dataset_name, id_filename=None):
    '''
    Parameters
    ----------
    ...
    if dataset_name == 'HOSP':
        A = pd.read_csv('C:\\Users\\6\\PycharmProjects\\FORECASTING_GNN\\benchmark data\\HOSP\\hosp_adj.csv')
        dist_A = None
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method,_mean, _std, params_path, type):
    ...
    '''
    net.train(False)  # ensure dropout layers are in test mode
    data_target_tensor = data_target_tensor.squeeze()
    with torch.no_grad():
