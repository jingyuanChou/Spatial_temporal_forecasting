...
def read_and_generate_dataset(dataset_name,data,len_input, graph_signal_matrix_filename,
    ...
    if dataset_name != 'HOSP':
        ...
        for idx in range(data_seq.shape[0]):
            ...
            if idx + len_input + num_for_predict < data_seq.shape[0]:
                sample_data_x = data_seq[idx: (idx + len_input):]
                sample_data_x = np.expand_dims(sample_data_x, axis=1)
                sample_data_x = np.expand_dims(sample_data_x, axis=0)
                sample_data_x = sample_data_x.transpose((0,3,2,1))
                sample.append(sample_data_x)

                sample_data_y = data_seq[(idx + len_input):(idx + len_input+num_for_predict),:]
                sample_data_y = np.expand_dims(sample_data_y, axis=1)
                sample_data_y = np.expand_dims(sample_data_y, axis=0)
                sample_data_y = sample_data_y.transpose((0,3,2,1))
                sample.append(sample_data_y)

                time_sample = np.expand_dims(np.array([idx + len_input]), axis=0)  # (1,1)
                sample.append(time_sample)
    ...
    split_line2 = int(len(all_samples) * 0.85)
...
parser.add_argument("--config", default='configurations/HOSP_astgcn.conf', type=str,
...
graph_signal_matrix_filename = data_config['graph_signal_matrix_filename']
if dataset_name == 'HOSP':
    data = pd.read_csv('C:\\Users\\6\\PycharmProjects\\FORECASTING_GNN\\benchmark data\\HOSP\\hosp_weekly_filt_case_data.csv')
    hospitalization = data.reset_index()
...
all_data = read_and_generate_dataset(dataset_name,data,len_input, graph_signal_matrix_filename, 0, 0, num_of_hours, num_for_predict, points_per_hour=points_per_hour, save=True)
